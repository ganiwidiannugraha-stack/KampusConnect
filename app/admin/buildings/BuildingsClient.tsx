...
export function BuildingsClient({ initialBuildings }: { initialBuildings: any[] }) {
  ...
  const filteredBuildings = initialBuildings.filter(b => {
    if (searchQuery.trim() !== '') {
      if (!b.nama_gedung.toLowerCase().includes(searchQuery.toLowerCase())) {
        return false;
      }
    }
    return true;
  ...
  const paginatedBuildings = filteredBuildings.slice((safePage - 1) * pageSize, safePage * pageSize);
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!window.confirm(`Yakin ingin menghapus gedung "${name}"? Semua ruangan di dalam gedung ini mungkin akan ikut terhapus.`)) return;
    
    setIsDeleting(id);
    const result = await deleteBuilding(id, name);
    if (result.success) {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left min-w-[600px]">
            <thead className="bg-muted/50 text-muted-foreground font-bold uppercase tracking-wider text-[11px]">
              <tr>
                <th className="px-6 py-4">Nama Gedung</th>
              ...
              {paginatedBuildings.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-muted-foreground font-medium">
                    Belum ada data gedung yang cocok.
                  </td>
                </tr>
              ) : (
                paginatedBuildings.map((building) => (
                  <tr key={building.id_gedung} className="hover:bg-muted/30 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="font-bold text-foreground">{building.nama_gedung}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-muted-foreground font-medium">{building.lokasi || '-'}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button 
                          onClick={() => handleDelete(building.id_gedung, building.nama_gedung)}
                          disabled={isDeleting === building.id_gedung}
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-border flex items-center justify-between bg-muted/10">
            <div className="text-xs text-muted-foreground">
              Menampilkan <span className="font-bold text-foreground">{paginatedBuildings.length > 0 ? (safePage - 1) * pageSize + 1 : 0}</span> - <span className="font-bold text-foreground">{Math.min(safePage * pageSize, filteredBuildings.length)}</span> dari <span className="font-bold text-foreground">{filteredBuildings.length}</span> gedung
            </div>
