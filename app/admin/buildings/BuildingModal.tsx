...
type BuildingModalProps = {
  building?: any; // null if creating
  isOpen: boolean;
  onClose: () => void;
};

export function BuildingModal({ building, isOpen, onClose }: BuildingModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!e.currentTarget.checkValidity()) {
      toast.error("Harap isi nama gedung terlebih dahulu.");
      return;
    }
    setIsSaving(true);

    const formData = new FormData(e.currentTarget);
    const result = await saveBuilding(formData, building?.id_gedung);
      ...
      <div className="relative bg-card w-full max-w-md rounded-2xl shadow-2xl border border-border overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form ref={formRef} noValidate onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-foreground mb-1.5">Nama Gedung</label>
              <input 
                name="name" 
                defaultValue={building?.nama_gedung} 
                required 
                placeholder="Misal: Gedung Kuliah Bersama, Rektorat"
            ...
            <div>
              <label className="block text-sm font-bold text-foreground mb-1.5">Lokasi / Alamat</label>
              <input 
                name="location" 
                defaultValue={building?.lokasi} 
                placeholder="Misal: Sayap Barat Kampus"
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-medium"
              />
            </div>
          </div>
