...
export function FacilityModal({ facility, isOpen, onClose }: FacilityModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!e.currentTarget.checkValidity()) {
      toast.error("Harap isi nama fasilitas terlebih dahulu.");
      return;
    }
    setIsSaving(true);

    const formData = new FormData(e.currentTarget);
    const result = await saveFacility(formData, facility?.id_fasilitas);

    if (result.success) {
      toast.success(facility ? 'Fasilitas berhasil diperbarui' : 'Fasilitas berhasil ditambahkan');
      onClose();
    } else {
      ...
      <div className="relative bg-card w-full max-w-md rounded-2xl shadow-2xl border border-border overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form ref={formRef} noValidate onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-foreground mb-1.5">Nama Fasilitas</label>
              <input 
                name="name" 
                defaultValue={facility?.nama_fasilitas} 
                required 
                placeholder="Misal: Proyektor LCD, Papan Tulis"
            ...
            <div>
              <label className="block text-sm font-bold text-foreground mb-1.5">Ikon (Optional)</label>
              <input 
                name="icon" 
                defaultValue={facility?.icon} 
                placeholder="Misal: Projector, Wind, Speaker"
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-medium"
              />
              <p className="text-xs text-muted-foreground mt-2">Nama ikon dari pustaka Lucide Icons.</p>
            </div>
          </div>
