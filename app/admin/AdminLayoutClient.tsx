    ...
    <div className="flex min-h-screen bg-background text-foreground font-sans selection:bg-primary/20">
      ...
      >
        <div className="flex flex-col h-full py-6">
          ...
          {/* Logo / Header */}
          <div className={`px-6 mb-8 flex flex-col ${isCollapsed ? 'items-center' : 'items-start'} transition-all duration-300`}>
            <Link href="/admin" className="flex items-center">
              <div className="h-10 w-10 rounded-xl flex items-center justify-center shrink-0">
                <img src="/logo.png" alt="Logo" className="w-9 h-9 object-contain drop-shadow-sm" />
              </div>
              <span className={`text-xl font-black tracking-tighter text-foreground whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? 'max-w-0 opacity-0 ml-0' : 'max-w-[200px] opacity-100 ml-3'}`}>KampusConnect</span>
            </Link>
            
            <Link 
              href="/" 
              className={`mt-2 ml-[52px] w-fit p-2 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-300 shadow-sm ${isCollapsed ? 'hidden' : 'flex'}`}
          ...
          <div className="mt-auto pt-6 px-4 pb-4 overflow-hidden">
            <LogoutButton className={`flex w-full items-center p-3 rounded-xl text-red-500 hover:bg-red-500/10 transition-colors ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
              ...
              <span className={`font-bold text-sm whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'max-w-0 opacity-0 overflow-hidden' : 'max-w-[150px] opacity-100'}`}>
                Keluar
              </span>
            </LogoutButton>
          </div>
